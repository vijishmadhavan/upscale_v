        ...
        if len(samples) > 0:
            ...
            for sample in batches:
                ...
                if vae_dtype != sample.dtype:
                    # Extract device type only (cuda, mps, cpu) for autocast
                    device_type = str(device).split(':')[0]
                    with torch.autocast(device_type, sample.dtype, enabled=True):
                        if use_sample:
        ...
        if len(latents) > 0:
            ...
            for i, latent in enumerate(latents):
                ...
                if vae_dtype != latent.dtype:
                    # Extract device type only (cuda, mps, cpu) for autocast
                    device_type = str(device).split(':')[0]
                    with torch.autocast(device_type, latent.dtype, enabled=True):
