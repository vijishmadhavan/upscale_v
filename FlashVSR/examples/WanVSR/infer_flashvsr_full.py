...
# Add FlashVSR root to Python path for Colab compatibility
script_dir = os.path.dirname(os.path.abspath(__file__))
flashvsr_root = os.path.abspath(os.path.join(script_dir, '../..'))
...
def main():
    parser = argparse.ArgumentParser(description='FlashVSR Full Inference')
    parser.add_argument('--input', '-i', type=str, nargs='+', help='Input video path(s) or folder(s)')
    ...
    parser.add_argument('--sparse_ratio', type=float, default=2.0, help='Sparse ratio (1.5 or 2.0)')
    args = parser.parse_args()

    RESULT_ROOT = args.output
    os.makedirs(RESULT_ROOT, exist_ok=True)

    # Use provided inputs or default examples
    if args.input:
        inputs = args.input
    else:
        inputs = [
