...
def process_with_realesrgan_streaming(frames_input, args, output_path, fps, debug=False, audio_path=None):
    ...
    print(f"🎬 Processing {total_frames} frames with {model_name} (streaming mode)")
...
def process_with_realesrgan(frames_tensor, args, debug=False):
    ...
    print(f"🎬 Processing {total_frames} frames with {model_name} (legacy mode)")
    if debug and args.realesrgan_tile > 0:
        print(f"🔲 Using tiled processing (tile size: {args.realesrgan_tile})")
...
def process_with_hybrid_realesrgan_4x(frames_input, args, output_path, fps, debug=False, audio_path=None):
    ...
    print(f"🎬 Processing {total_frames} frames with {model_name} (hybrid 4x mode)")
    inference_start = time.time()
