...
# Pre-parse cuda_device for SeedVR2
...
extract_frames_from_video = inference_cli_new.extract_frames_from_video
...
def detect_text_in_video(video_path, max_frames_to_check=None, debug=False, model_dir=None, script_dir=None):
    ...
    try:
        ...
        return {}, 0.0  # Return empty dict if error occurs
...
def save_tensor_to_video_streaming(frames_tensor, output_path, fps, debug=False):
    ...
    # Create video writer with fast H.264 codec
    # Try hardware-accelerated codecs first, fall back to mp4v if needed
    writer = None
    for codec in ['h264', 'avc1', 'H264', 'X264', 'mp4v']:
        ...
        if writer.isOpened():
            if debug:
                print(f"📹 Using codec: {codec} for video assembly")
            break
...
def process_seedvr2_adaptive(frames_tensor, args, original_fps, device_list, seedvr2_args, debug=False, threshold=None):
    ...
    if frame_count <= safe_frame_limit:
        ...
        # Create video writer with fast codec for output
        writer = None
        for codec in ['h264', 'avc1', 'H264', 'X264', 'mp4v']:
            fourcc = cv2.VideoWriter_fourcc(*codec)
            writer = cv2.VideoWriter(temp_file, fourcc, original_fps, (out_w, out_h))
            if writer.isOpened():
                if debug:
                    print(f"📹 SeedVR2 batch writer using codec: {codec}")
                break

        if not writer or not writer.isOpened():
            raise RuntimeError(f"Failed to open video writer for {temp_file}")
...
def apply_selective_blur_to_frames(frames_tensor, blur_type, blur_strength_text, blur_strength_rest,
                                   max_resolution, text_regions, text_coverage_threshold=20.0, debug=False):
    """Apply selective blur based on text regions - lower blur for small text, normal blur for rest

    Args:
        frames_tensor: Input frames
        blur_type: Type of blur
        blur_strength_text: Blur strength for text regions (e.g., 1.0)
        blur_strength_rest: Blur strength for non-text regions (e.g., 2.5)
        max_resolution: Max resolution for resize
        text_regions: Dict of {frame_idx: [polygons]}
        text_coverage_threshold: If text coverage > this %, use uniform blur (default: 20%)
        debug: Debug flag
    """
    if debug:
        print(f"\n🔄 Preparing frames with selective blur for SeedVR2...")
    ...
    for i, frame in enumerate(frames_np):
        ...
        # Apply selective blur if requested
        if blur_type != 'none' and (blur_strength_text > 0 or blur_strength_rest > 0):
            # Check if this frame has text
            if i in text_regions and len(text_regions[i]) > 0:
                # Calculate text coverage for this frame
                frame_h, frame_w = frame.shape[:2]
                frame_area = frame_h * frame_w
                text_area = 0

                for poly in text_regions[i]:
                    poly_array = np.array(poly, dtype=np.int32)
                    # Scale polygons if frame was resized
                    if resize_needed:
                        scale_x = new_width / W
                        scale_y = new_height / H
                        poly_array = (poly_array * [scale_x, scale_y]).astype(np.int32)

                    x_min = max(0, int(np.min(poly_array[:, 0])))
                    y_min = max(0, int(np.min(poly_array[:, 1])))
                    x_max = min(frame_w, int(np.max(poly_array[:, 0])))
                    y_max = min(frame_h, int(np.max(poly_array[:, 1])))
                    text_area += (x_max - x_min) * (y_max - y_min)

                coverage_percent = (text_area / frame_area) * 100

                # If text covers major portion, use uniform blur
                if coverage_percent >= text_coverage_threshold:
                    # Convert RGB to BGR for OpenCV blur functions
                    frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                    frame_bgr = apply_blur_to_frame(frame_bgr, blur_type, blur_strength_rest)
                    frame = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
                else:
                    # Apply selective blur: lower for text, higher for rest
                    frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                    ...
                    if resize_needed:
                        ...
                        for poly in text_regions[i]:
                            ...
                            scaled_polygons.append(poly_array)
                    else:
                        scaled_polygons = text_regions[i]

                    # Create mask for text regions
                    text_mask = create_text_mask_from_polygons(
                        scaled_polygons,
                        frame_w, frame_h, padding=5
                    )

                    # Blur text regions with lower strength
                    text_blurred = apply_blur_to_frame(frame_bgr.copy(), blur_type, blur_strength_text)

                    # Blur non-text regions with higher strength
                    rest_blurred = apply_blur_to_frame(frame_bgr.copy(), blur_type, blur_strength_rest)

                    # Composite: use text blur for text regions, rest blur for non-text
                    frame_bgr = rest_blurred.copy()
                    frame_bgr[text_mask > 0] = text_blurred[text_mask > 0]

                    frame = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
            else:
                # No text in this frame, apply normal blur
                frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                frame_bgr = apply_blur_to_frame(frame_bgr, blur_type, blur_strength_rest)
                frame = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    ...
    if debug:
        print(f"🌀 Applied selective blur (text: {blur_strength_text}, rest: {blur_strength_rest})")
        print(f"📊 Output shape: {result_tensor.shape}")
...
class HYPIRUpsampler:
    """Wrapper to use HYPIR as a background upsampler similar to RealESRGAN"""
    
    def __init__(self, model, scale=2, patch_size=512, stride=256, device='cuda'):
        self.model = model
        self.scale = scale
    ...
    def enhance(self, img_rgb, prompt='', outscale=None):
        """
        Enhance image using HYPIR
        Args:
            img_rgb: RGB numpy array
            prompt: Text prompt for enhancement
            outscale: Output scale (overrides default scale)
        Returns:
            Enhanced RGB numpy array, None
        """
        # Use provided outscale or default scale
        scale = int(outscale) if outscale is not None else int(self.scale)
        
        # Convert to PIL
        pil_img = Image.fromarray(img_rgb)
        
        # Convert to tensor and move to device
        img_tensor = self.to_tensor(pil_img).unsqueeze(0).to(self.device)
        
        # Enhance with HYPIR - NO FALLBACK
        enhanced_pil = self.model.enhance(
            lq=img_tensor,
            prompt=prompt,
            upscale=scale,
        ...
        return enhanced_rgb, None


def create_text_mask_from_polygons(polygons, width, height, padding=5):
    """Create binary mask from text polygons
    ...
    Returns:
        Binary mask (uint8): 255 for text regions, 0 for background
    """
    ...
    for poly in polygons:
        poly_array = np.array(poly, dtype=np.int32)
        # Add padding
        x_min = max(0, int(np.min(poly_array[:, 0])) - padding)
        y_min = max(0, int(np.min(poly_array[:, 1])) - padding)
        x_max = min(width, int(np.max(poly_array[:, 0])) + padding)
        y_max = min(height, int(np.max(poly_array[:, 1])) + padding)
...
def create_streaming_video_writer(output_path, width, height, fps, debug=False, with_audio=False):
    ...
    if not writer or not writer.isOpened():
        raise RuntimeError(f"Failed to open video writer for {actual_path}")

    if debug:
        print(f"📹 Using OpenCV VideoWriter: {width}x{height} @ {fps}fps, codec: {codec_used}")
...
def process_with_realesrgan_streaming(frames_tensor, args, output_path, fps, debug=False, audio_path=None):
    ...
    # Convert tensor to numpy for RealESRGAN
    frames_np = frames_tensor.cpu().numpy()
    frames_np = (frames_np * 255.0).astype(np.uint8)
    ...
    output_width = input_width * args.realesrgan_scale
    output_height = input_height * args.realesrgan_scale
    
    # Setup RealESRGAN (same as before)
    if args.realesrgan_model is None:
        if args.realesrgan_scale == 2:
            model_candidates = [
                os.path.join(args.model_dir, 'RealESRGAN_x2plus.pth'),
                '/content/ComfyUI-SeedVR2_VideoUpscaler/models/RealESRGAN_x2plus.pth',
                'D:\\GFPGAN\\gfpgan\\weights\\RealESRGAN_x2plus.pth',
                '/mnt/d/GFPGAN/gfpgan/weights/RealESRGAN_x2plus.pth'
            ]
        else:  # scale == 4
            model_candidates = [
                os.path.join(args.model_dir, 'RealESRGAN_x4plus.pth'),
                '/content/ComfyUI-SeedVR2_VideoUpscaler/models/RealESRGAN_x4plus.pth',
                'D:\\GFPGAN\\gfpgan\\weights\\RealESRGAN_x4plus.pth',
                '/mnt/d/GFPGAN/gfpgan/weights/RealESRGAN_x4plus.pth'
            ]
        ...
        for candidate in model_candidates:
            if os.path.exists(candidate):
                model_path = candidate
                if debug:
                    print(f"📁 Found RealESRGAN model: {model_path}")
                break
        
        if model_path is None:
            raise FileNotFoundError(f"Could not find RealESRGAN_x{args.realesrgan_scale}plus.pth")
    else:
    ...
    if debug:
        ...
        print(f"🔧 Setting up RealESRGAN_x{args.realesrgan_scale}plus...")
    ...
    # Disable torch.compile for RealESRGAN due to cudaMallocAsync compatibility issues
    # The speedup is minimal compared to the stability issues it causes
    if debug and torch.__version__ >= '2.0.0':
        print(f"📊 torch.compile disabled for RealESRGAN (cudaMallocAsync compatibility)")
    ...
    if debug:
        print(f"🎬 Processing {total_frames} frames with RealESRGAN x{args.realesrgan_scale} (streaming mode)")
        if args.realesrgan_tile > 0:
    ...
    try:
        ...
        for i, frame in enumerate(frames_np):
            ...
            try:
                ...
                # Upscale
                output, _ = upsampler.enhance(frame_bgr, outscale=args.realesrgan_scale)
                
                # Write directly to video (output is already BGR for cv2)
                ...
                if "out of memory" in str(e).lower():
                    print(f"⚠️ GPU OOM at frame {i}. Try using --realesrgan-tile 256")
                    raise
                else:
                    print(f"⚠️ Error at frame {i}, using fallback upscaling: {e}")
                    h, w = frame.shape[:2]
                    upscaled = cv2.resize(frame, (w * args.realesrgan_scale, h * args.realesrgan_scale), 
...
def process_with_realesrgan(frames_tensor, args, debug=False):
    if debug:
        print(f"\n🎯 Starting RealESRGAN processing")
        print(f"📊 Input tensor shape: {frames_tensor.shape}")
    ...
    if args.realesrgan_model is None:
        if args.realesrgan_scale == 2:
            model_candidates = [
                os.path.join(args.model_dir, 'RealESRGAN_x2plus.pth'),  # In SeedVR2 models dir
                '/content/ComfyUI-SeedVR2_VideoUpscaler/models/RealESRGAN_x2plus.pth',  # Default path
                'D:\\GFPGAN\\gfpgan\\weights\\RealESRGAN_x2plus.pth',
                '/mnt/d/GFPGAN/gfpgan/weights/RealESRGAN_x2plus.pth'
            ]
        else:  # scale == 4
            model_candidates = [
                os.path.join(args.model_dir, 'RealESRGAN_x4plus.pth'),  # In SeedVR2 models dir
                '/content/ComfyUI-SeedVR2_VideoUpscaler/models/RealESRGAN_x4plus.pth',  # Default path
                'D:\\GFPGAN\\gfpgan\\weights\\RealESRGAN_x4plus.pth',
                '/mnt/d/GFPGAN/gfpgan/weights/RealESRGAN_x4plus.pth'
            ]
    ...
    """Legacy RealESRGAN processing - only used for intermediate stages (not recommended)"""
    print(f"⚠️ WARNING: Using legacy RealESRGAN processing - this accumulates frames in memory!")
    print(f"💡 Streaming mode is recommended for all video processing")
    
    # Simple processing for cases where we need tensor output (rare)
    processed_frames = []
    total_frames = len(frames_np)
    ...
    if debug:
        print(f"🎬 Processing {total_frames} frames with RealESRGAN x{args.realesrgan_scale} (legacy mode)")
        if args.realesrgan_tile > 0:
            print(f"🔲 Using tiled processing (tile size: {args.realesrgan_tile})")
    ...
    if debug:
        print(f"✅ RealESRGAN processing complete")
        print(f"📊 Output shape: {result_tensor.shape}")
...
Supported resolutions:
  ...
  - Custom: WIDTHxHEIGHT
    ...
    seedvr2_advanced.add_argument("--flashvsr_max_frames", type=int, default=1000,
                        help="Maximum frames for FlashVSR in creative mode (default: 1000 frames)")
...
def process_with_hybrid_realesrgan_4x(frames_tensor, args, output_path, fps, debug=False, audio_path=None):
    ...
    # Setup RealESRGAN 2x model (same as standard but force 2x)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # Load RealESRGAN 2x model
    ...
    if not model_path:
        raise FileNotFoundError("RealESRGAN_x2plus.pth not found")
    ...
    # Disable torch.compile for RealESRGAN due to cudaMallocAsync compatibility issues
    # (cudaMallocAsync is needed for SeedVR2)
    ...
    # Try fast codecs first for RealESRGAN output
    video_writer = None
    for codec in ['h264', 'avc1', 'H264', 'X264', 'mp4v']:
        fourcc = cv2.VideoWriter_fourcc(*codec)
        video_writer = cv2.VideoWriter(actual_path, fourcc, fps, (output_width, output_height))
        if video_writer.isOpened():
            if debug:
                print(f"📹 RealESRGAN hybrid writer using codec: {codec}")
            break

    if not video_writer or not video_writer.isOpened():
        raise RuntimeError(f"Failed to open video writer for {actual_path}")
    
    # Process each frame: RealESRGAN 2x + Lanczos 2x
    inference_start = time.time()
    ...
    for i, frame in enumerate(frames_np):
        try:
            ...
            # Step 1: RealESRGAN 2x upscaling
            realesrgan_output, _ = upsampler.enhance(frame_bgr, outscale=2)
            
            # Step 2: Lanczos 2x upscaling (very fast)
            final_output = cv2.resize(realesrgan_output, (output_width, output_height), interpolation=cv2.INTER_LANCZOS4)
            
            # Write directly to video (streaming)
            ...
            fallback_frame = cv2.resize(frame_bgr, (output_width, output_height), interpolation=cv2.INTER_LANCZOS4)
...
def main():
    ...
    global todos
    todos = [
        {"id": "1", "content": "Create new pipeline file with GFPGAN+HyPIR for Stage 1", "status": "completed", "priority": "high"},
        {"id": "2", "content": "Import HyPIR-related modules and classes", "status": "completed", "priority": "high"},
        {"id": "3", "content": "Modify process_with_gfpgan to use HyPIR instead of RealESRGAN", "status": "completed", "priority": "high"},
        {"id": "4", "content": "Add HyPIR-specific command line arguments", "status": "completed", "priority": "high"},
        {"id": "5", "content": "Update background upsampling logic to use HyPIR", "status": "completed", "priority": "medium"},
        {"id": "6", "content": "Test the integration points between stages", "status": "in_progress", "priority": "medium"}
    ]
    ...
    if not hasattr(args, 'model_dir') or args.model_dir is None:
        ...
        for candidate in model_dir_candidates:
            if os.path.exists(candidate):
                ...
                break
        if not hasattr(args, 'model_dir') or args.model_dir is None:
            args.model_dir = "seedvr2_models"  # Fallback
    ...
    try:
        ...
        # Extract frames using new inference_cli (no blur/resize - handled separately)
        ...
        if args.debug:
            ...
            print(f"📊 GPU memory after extraction: {allocated:.1f}GB allocated, {reserved:.1f}GB reserved")
        
        # Stage 1.5: Resize input to max 620p for optimal processing
        resize_start = time.time()
        frames_tensor = resize_input_frames_to_720p(frames_tensor, original_debug)
        resize_time = time.time() - resize_start
        
        if original_debug:
            print(f"⏱️ Input resize time: {resize_time:.2f}s")
            print(f"📊 Frames after resize: {frames_tensor.shape}")
        ...
        if args.pipeline_mode in ["all", "realesrgan-only", "seedvr2-realesrgan"]:
            ...
            if args.skip_realesrgan:
                ...
                # Handle both tensor and file input for RealESRGAN
                if isinstance(current_frames, str):
                    # Process from temp file
                    if original_debug:
                        print(f"📁 Processing RealESRGAN from temp file: {current_frames}")

                    # Extract frames from temp file and process with RealESRGAN streaming
                    temp_frames, _ = extract_frames_from_video(current_frames)
                    ...
                    if args.realesrgan_scale == 4:
                        final_result, realesrgan_loading_time, realesrgan_inference_time = process_with_hybrid_realesrgan_4x(
                            temp_frames, args, streaming_output_path, original_fps, args.debug, audio_path
                        )
                    else:
                    ...
                    # Cleanup temp file
                    if os.path.exists(current_frames):
                        try:
                            os.unlink(current_frames)
                            if original_debug:
                                print(f"🗑️ Cleaned up temp file: {current_frames}")
                    ...
                    # Process from tensor (original path)
                    if args.realesrgan_scale == 4:
                        final_result, realesrgan_loading_time, realesrgan_inference_time = process_with_hybrid_realesrgan_4x(
                ...
                # Stage 4.5: Apply Lanczos 2x after RealESRGAN (for 4K targets)
                if args.use_lanczos_upscale:
                    if original_debug:
                        print(f"\n🔄 Stage 4.5: Lanczos 2x upscaling after RealESRGAN")

                    lanczos_start = time.time()

                    # RealESRGAN already saved to streaming_output_path
                    # Need to read it back and apply Lanczos
                    temp_frames, _ = extract_frames_from_video(streaming_output_path)

                    # Apply Lanczos 2x upscaling (doubles the RealESRGAN output)
                    lanczos_upscale_frames_streaming(
                        temp_frames,
                        args.target_width,
                        args.target_height,
                        streaming_output_path,  # Overwrite the RealESRGAN output
                        original_fps,
                        original_debug,
                        audio_path
                    )
                    ...
                    lanczos_time = time.time() - lanczos_start
                    if original_debug:
                        print(f"⏱️ Lanczos 2x upscaling time: {lanczos_time:.2f}s")
        ...
        print(f"\n✅ Completed in {int(total_time)} seconds")
