...
def _setup_realesrgan(model_dir, target_scale, tile=0, explicit_model=None, debug=False):
    """Create RealESRGAN upsampler, preferring fast x4v3 when available.

    Tries realesr-general-x4v3.pth (SRVGGNetCompact, ~600K params, ~10x faster)
    first, falls back to RealESRGAN_x2plus/x4plus (RRDBNet, ~16.7M params).
    ...
    # Auto-detect: prefer fast x4v3 when available
    if SRVGGNetCompact is not None:
        x4v3_path = os.path.join(model_dir, 'realesr-general-x4v3.pth')
        if os.path.exists(x4v3_path):
            model = SRVGGNetCompact(num_in_ch=3, num_out_ch=3, num_feat=64, num_conv=32, upscale=4, act_type='prelu')
            upsampler = RealESRGANer(
                scale=4, model_path=x4v3_path, model=model,
                tile=tile, tile_pad=32, pre_pad=0, half=half
            )
            if debug:
                print(f"⚡ Using fast RealESRGAN x4v3 (SRVGGNetCompact, ~600K params)")
            return upsampler, 'realesr-general-x4v3'

    # Fallback to RRDB model
    rrdb_name = f'RealESRGAN_x{target_scale}plus.pth'
    model_path = os.path.join(model_dir, rrdb_name)

    if not os.path.exists(model_path):
        raise FileNotFoundError(
            f"Could not find RealESRGAN model weights. "
            f"Place realesr-general-x4v3.pth or {rrdb_name} in {model_dir}"
        )
    ...
    if debug:
        print(f"📦 Using RealESRGAN x{target_scale}plus (RRDBNet, ~16.7M params)")
    return upsampler, f'RealESRGAN_x{target_scale}plus'


def process_with_realesrgan_streaming(frames_input, args, output_path, fps, debug=False, audio_path=None):
    ...
    # Setup RealESRGAN (prefers fast x4v3 if available, falls back to RRDB)
    ...
    try:
        ...
        for i in range(total_frames):
            ...
            # Read frame — from video file or numpy array
            ...
            try:
                ...
                if _progress:
                    _progress.stage('realesrgan', (i + 1) / total_frames)
                if debug and (i + 1) % 50 == 0:
                ...
                if "out of memory" in str(e).lower():
                    ...
                    upscaled_bgr = cv2.resize(frame_bgr, (w * args.realesrgan_scale, h * args.realesrgan_scale),
...
def process_with_hybrid_realesrgan_4x(frames_input, args, output_path, fps, debug=False, audio_path=None):
    ...
    # Setup RealESRGAN 2x (prefers fast x4v3 if available, falls back to RRDB)
    upsampler, model_name = _setup_realesrgan(
        ...
        if args.pipeline_mode in ["all", "realesrgan-only", "seedvr2-realesrgan"]:
            ...
            if args.skip_realesrgan:
                ...
                if args.use_lanczos_upscale:
                    ...
                    # File-to-file Lanczos upscaling — zero RAM accumulation
                    # lanczos_upscale_frames_streaming auto-detects file path input
                    lanczos_temp = streaming_output_path + ".lanczos_temp.mp4"
                    lanczos_upscale_frames_streaming(
                        streaming_output_path,  # Pass file path directly
                        args.target_width,
                        args.target_height,
                        lanczos_temp,
                        original_fps,
                    ...
                    os.replace(lanczos_temp, streaming_output_path)
