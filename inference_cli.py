...
def apply_blur_to_frame(frame: np.ndarray, blur_type: str = 'gaussian', blur_strength: float = 1.0) -> np.ndarray:
    """
    ...
    Returns:
        Blurred frame (same format as input)
    """
    if blur_strength <= 0:
        return frame
...
def _worker_process(
    ...
    # Suppress tqdm progress bars in quiet mode
    if shared_args.get("quiet", False):
        os.environ["TQDM_DISABLE"] = "1"

    import torch

    # Create debug instance for this worker
    worker_debug = Debug(enabled=shared_args["debug"], quiet=shared_args.get("quiet", False))
